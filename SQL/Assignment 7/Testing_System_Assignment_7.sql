...
-- trước 1 năm trước
DROP TRIGGER IF EXISTS Check_Group_CreateDate;
DELIMITER $$
CREATE TRIGGER Check_Group_CreateDate
...
FOR EACH ROW
	BEGIN
        ...
        SET MESSAGE_TEXT = 'Khong the tao Group do ngay tao ko hop le';
        END IF;
    END $$
DELIMITER ;

INSERT INTO `testing_system_assignment_7`.`group` (`GroupID`, `GroupName`, `CreatorID`, `CreateDate`) VALUES ('11', 'abc', '5', '2019-04-10 00:00:00');

-- Question 2: Tạo trigger Không cho phép người dùng thêm bất kỳ user nào vào
-- department "Sale" nữa, khi thêm thì hiện ra thông báo "Department
-- "Sale" cannot add more user"

DROP TRIGGER IF EXISTS Ko_Them_User;
DELIMITER $$
CREATE TRIGGER Ko_Them_User
...
INSERT INTO `testing_system_assignment_7`.`account` (`AccountID`, `Email`, `Username`, `FullName`, `DepartmentID`, `PositionID`, `CreateDate`) VALUES ('257', 'rgadhd', 'htfsrys', 'hahahaha', '8', '2', '2020-12-03 00:00:00');

-- Question 3: Cấu hình 1 group có nhiều nhất là 5 user
DROP TRIGGER IF EXISTS Group_5user;
DELIMITER $$
...
FOR EACH ROW
	BEGIN
		DECLARE V_CountAcc MEDIUMINT;
            ...
            FROM		groupaccount
            GROUP BY	GroupID
            HAVING		GroupID = NEW.GroupID
           ;
		IF V_CountAcc >= 18 THEN
        SIGNAL SQLSTATE '12345'
        SET MESSAGE_TEXT = 'Group nay da co tren 18 ng, ko the them';
        END IF;
    END $$
DELIMITER ;

INSERT INTO `testing_system_assignment_7`.`groupaccount` (`GroupID`, `AccountID`, `JoinDate`) VALUES ('1', '5', '2020-05-11 00:00:00');
...
DROP TRIGGER IF EXISTS Delete_Account_Admin;

DELIMITER $$
CREATE TRIGGER Delete_Account_Admin
BEFORE DELETE ON `account`
FOR EACH ROW
	BEGIN
		DECLARE	V_accountID MEDIUMINT;
			SELECT 	AccountID INTO V_accountID
			FROM	`account`
			WHERE	email = OLD.email;
        IF OLD.email = 'admin@gmail.com' THEN
			SIGNAL SQLSTATE '12345'
			SET MESSAGE_TEXT = 'Day la User Admin, Ko dc xoa';
...
DELETE FROM `account` WHERE email = 'reden6z@toplist.cz';
...
FOR EACH ROW
	BEGIN
		DECLARE	V_WaitingRoom MEDIUMINT;
...
INSERT INTO `testing_system_assignment_7`.`account` (`AccountID`, `Email`, `Username`, `FullName`, `PositionID`, `CreateDate`) VALUES ('257', 'jmountai4n6y@macromedia.com', 'tlackney6wq', 'Paton Mattys24', '1', '2020-12-03 00:00:00');

-- Question 7: Cấu hình 1 bài thi chỉ cho phép user tạo tối đa 4 answers cho mỗi
-- question, trong đó có tối đa 2 đáp án đúng.

DROP TRIGGER IF EXISTS Check_Insert_Answer;
DELIMITER $$
CREATE TRIGGER Check_Insert_Answer
BEFORE INSERT ON `answer`
FOR EACH ROW
	BEGIN
		DECLARE	V_Count_Answer MEDIUMINT;
        DECLARE	V_Count_Answer_isCorrect MEDIUMINT;
			SELECT 	count(*) INTO V_Count_Answer
			FROM	Department
			WHERE	DepartmentName = 'Waiting Department';
       IF NEW.DepartmentID IS NULL THEN
       SET NEW.DepartmentID = V_WaitingRoom;
