...
PositionName ENUM('Dev','Test','Scrum Master','PM','developer') NOT NULL UNIQUE KEY 
);
...
-- INSERT TABLE
INSERT INTO Position (PositionName ) VALUES ('Dev' ),('Test' ),('Scrum Master'),('PM' ), ('Developer');
...
insert into account (Email, Username, FullName, DepartmentID, PositionID, CreateDate) values
('cheadleyq@usnews.com', 'cheadleyq', 'Cynthea Headley', 5, 5, '2020-09-23');
insert into account (Email, Username, FullName, DepartmentID, PositionID, CreateDate) values
...
insert into account (Email, Username, FullName, DepartmentID, PositionID, CreateDate) values
('fdallau@smh.com.au', 'fdallau', 'Ferdie Dalla', 1, 5, '2020-09-17');
insert into account (Email, Username, FullName, DepartmentID, PositionID, CreateDate) values
...
DROP PROCEDURE IF EXISTS Get_DepName;

DELIMITER $$
CREATE PROCEDURE Get_DepName (IN IN_Name VARCHAR (30))
	BEGIN
		SELECT	*
        FROM	`account` A
        JOIN	department D ON A.DepartmentID = D.DepartmentID
        WHERE	DepartmentName = IN_Name
        ;
    END $$
DELIMITER ;

CALL Get_DepName ('Sale');

-- Question 2: Tạo store để in ra số lượng account trong mỗi group

DROP PROCEDURE IF EXISTS Get_SL_AccOfGroup;

DELIMITER $$
CREATE PROCEDURE Get_SL_AccOfGroup ()
	BEGIN
		SELECT		GroupID, ( 	SELECT GroupName 
								FROM `Group` 
                                WHERE GroupID = GA.GroupID) AS G_Name, count(*) AS SL
...
CALL Get_SL_AccOfGroup();

-- Question 3: Tạo store để thống kê mỗi type question có bao nhiêu question được tạo trong tháng hiện tại

DROP PROCEDURE IF EXISTS Get_SL_QinM;

DELIMITER $$
CREATE PROCEDURE Get_SL_QinM ()
	BEGIN
		SELECT		count(TypeID)
		FROM		question
...
DROP PROCEDURE IF EXISTS Get_TypeID;

DELIMITER $$
CREATE PROCEDURE Get_TypeID (OUT Out_TypeID MEDIUMINT)
	BEGIN
		SELECT		TypeID INTO Out_TypeID
		FROM		question
		GROUP BY	TypeID
		HAVING		count(*) = (	SELECT	max(DemQ) AS Max
									FROM	(	SELECT		count(*) AS DemQ
												FROM		question
												GROUP BY	TypeID) AS A)
		;
	End $$
DELIMITER ;
SET @V_ID = 0;
CALL Get_TypeID (@V_ID);
...
-- Nhập vào username và trả ra ID của user
DROP FUNCTION IF EXISTS User_ID;

DELIMITER $$
CREATE FUNCTION User_ID (In_User_Name VARCHAR (50)) RETURNS MEDIUMINT
BEGIN
	DECLARE ID MEDIUMINT;
...
DROP PROCEDURE IF EXISTS Tim_UserName_Or_GroupName;

DELIMITER $$
CREATE PROCEDURE Tim_UserName_Or_GroupName (IN IN_Kytu VARCHAR (50))
BEGIN
	SELECT	A.Username
	FROM	`account` A
	WHERE	A.Username LIKE CONCAT("%",IN_Kytu,"%")
	UNION
	SELECT	G.GroupName
	FROM	`Group` G
	WHERE	G.GroupName LIKE CONCAT("%",IN_Kytu,"%");
END$$

DELIMITER ;

CALL Tim_UserName_Or_GroupName ('no');
...
DROP PROCEDURE IF EXISTS Insert_FullName_Email;

DELIMITER $$
CREATE PROCEDURE Insert_FullName_Email (IN Var_FullName VARCHAR (50), IN Var_Email VARCHAR (50))
BEGIN
