...
INSERT INTO GroupAccount (GroupID, AccountID, JoinDate) VALUES (11, NULL, '2020-04-10 00:00:00');
...
SELECT		*, count(D.DepartmentID) AS So_NV
FROM		`account` A
...
WITH	CTE_Q AS (	SELECT		count(EQ.QuestionID) AS So_Q
					FROM		exam E
...
FROM		examquestion
GROUP BY	QuestionID
HAVING		count(QuestionID) = (	SELECT	max(So_Q) AS Max
									FROM	CTE_Q)
;
...
SELECT		*, count(E.QuestionID)
FROM		examquestion E RIGHT JOIN	Question Q 
ON 			E.QuestionID = Q.QuestionID
GROUP BY	E.QuestionID
;

-- Question 8: Lấy ra Question có nhiều câu trả lời nhất
WITH	CTE_A AS (	SELECT		count(A.QuestionID) AS So_A
					FROM		Answer A
...
FROM		Answer
GROUP BY	QuestionID
HAVING		count(QuestionID) = (	SELECT	max(So_A) AS Max
									FROM	CTE_A)
;

-- Question 9: Thống kê số lượng account trong mỗi group
SELECT		G.GroupID, G.GroupName, count(G.CreatorID)
FROM		`account` A
...
WITH	CTE_M AS (	SELECT		count(P.PositionID) AS So_P
					FROM		`account` A
...
HAVING		count(A.PositionID) = (	SELECT	min(So_P) AS MIN
									FROM	CTE_M)
;

-- Question 11: Thống kê mỗi phòng ban có bao nhiêu dev, test, scrum master, PM
SELECT		D.DepartmentName, P.PositionName, count(P.PositionID) AS SoNV
FROM		`account`	A
JOIN		`Position` 	P ON A.PositionID = P.PositionID
JOIN		department	D ON A.DepartmentID = D.DepartmentID
...
SELECT 		G.GroupID, G.GroupName, COUNT(*) AS So_TV
FROM 		`Group` G
JOIN 		GroupAccount GA ON G.GroupID = GA.GroupID
GROUP BY	GA.GroupID
HAVING 		So_TV > 5
