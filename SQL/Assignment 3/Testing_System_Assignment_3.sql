...
WITH CTE_Department3 AS (SELECT	*
				FROM	`account`
                WHERE	DepartmentID = 3)
SELECT		*
FROM		`CTE_Department3`
WHERE		length(Fullname) = 	(	SELECT 		Max(length(fullname))
									FROM		`CTE_Department3`)
;
...
-- Question 6: Lấy ra tên group đã tham gia trước ngày 06/04/2020
SELECT	GroupName, CreateDate
FROM	`group`
WHERE	CreateDate < '2020-04-06'
;

SELECT	CreateDate	FROM	`group`;
...
-- Question 11: Lấy ra nhân viên có tên bắt đầu bằng chữ "D" và kết thúc bằng chữ "I"
SELECT		*
FROM		`account`
WHERE		FullName Like 'D%I'
;

-- Question 12: Xóa tất cả các exam được tạo trước ngày '2020-04-06'
DELETE		
FROM		exam
WHERE		CreateDate < '2020-04-06'
;
