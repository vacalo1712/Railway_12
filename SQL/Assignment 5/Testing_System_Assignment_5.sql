...
CREATE OR REPLACE VIEW So_Acc AS
	SELECT		count(G.GroupID) AS Dem
	FROM		`account` A
	JOIN		groupaccount G ON A.AccountID = G.AccountID
	GROUP BY	A.AccountID
;

SELECT		A.*, count(*) AS SoAcc
FROM		`Account` A
JOIN		GroupAccount G ON A.AccountID = G.AccountID
GROUP BY	A.AccountID
HAVING		SoAcc = ( 	SELECT	max(Dem) AS M
						FROM	So_acc)
;

-- Question 3: Tạo view có chứa câu hỏi có những content quá dài (content quá 300 từ được coi là quá dài) và xóa nó đi
CREATE OR REPLACE VIEW Char_Content AS
	SELECT	*
	FROM	Question
...
CREATE OR REPLACE VIEW Acc_Of_Dep AS
SELECT		D.DepartmentID, D.DepartmentName,count(A.AccountID) AS So_NV
FROM		`account` A
JOIN		department D ON A.DepartmentID = D.DepartmentID
GROUP BY	D.DepartmentID
HAVING		So_NV = (	SELECT	max(NV) AS Max
						FROM	(	SELECT		count(A.AccountID) AS NV
									FROM		`account` A
									JOIN		department D ON A.DepartmentID = D.DepartmentID
...
CREATE OR REPLACE VIEW Que_User AS
